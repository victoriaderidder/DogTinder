...
<?php
    ...
    if(!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== true){
        ...
        $query = "SELECT Name FROM Dog_Profile ORDER BY rand() LIMIT 1";
        ...
        $query = "SELECT Dog_ID FROM Dog_Profile WHERE Name ='$Dog_name'";
        $result = mysqli_query($link, $query);
        while($row = mysqli_fetch_row($result)) {
            $count = count($row);
            foreach($row as $cell){
        ...
        $query = "SELECT Description FROM Dog_Profile WHERE Name = '$Dog_name'";
        $result = mysqli_query($link, $query);
        while($row = mysqli_fetch_row($result)) {
            $count = count($row);
            foreach($row as $cell){
...
<body>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
             <div class="form-group">
			    <input type="submit" class="btn btn-primary" value="yes" name="likes">
            </div>
            <div class="form-group>">
                <input type="submit" class="btn btn-primary" value="no" name="dislikes">
            </div>
            <p>
                <a href="portfolio.php" class="btn btn-danger">HomePage</a>
            </p>
        </form>
</body>
</html>